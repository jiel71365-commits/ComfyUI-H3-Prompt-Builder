...
6. **漫剧：导演审阅**：分镜 JSON + 剧本原文 → 审阅后的分镜 JSON + 资产 JSON + 审阅报告；自动修正循环直到 PASS（默认最多 5 轮）。
...
- **导演审阅**：「漫剧：导演审阅」按规则文件检查覆盖率/轴线/连续性/时长对账/动作具体性/情绪外化/资产完整性/台词，FAIL 自动修正并复核直到 PASS；轮数受 `max_review_rounds` 限制。
...
| max_review_rounds | 5 | 导演审阅最大轮数；超过后交付当前分镜并附未决问题清单 |
